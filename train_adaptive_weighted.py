...
from data import fusion_adaptive_weighted_dataset
...
class WeightedDataPrefetcher(object):
    ...
    def preload(self):
        try:
            self.next_input, self.next_target, self.next_weight, _, _ = next(self.loader)
        ...
        with torch.cuda.stream(self.stream):
            ...
            self.next_weight = self.next_weight.cuda(non_blocking=True).float()

    def next(self):
        torch.cuda.current_stream().wait_stream(self.stream)
        image = self.next_input
        mask = self.next_target
        weight_map = self.next_weight
        self.preload()
        return image, mask, weight_map


def weighted_cross_entropy(logits, target, weight_map, ignore_index=255, eps=1e-6):
    if weight_map.dim() == 4:
        weight_map = weight_map.squeeze(1)

    per_pixel_loss = F.cross_entropy(logits, target, ignore_index=ignore_index, reduction='none')
    valid_mask = (target != ignore_index).float()
    effective_weight = weight_map * valid_mask
    normalizer = effective_weight.sum().clamp_min(eps)
    return (per_pixel_loss * effective_weight).sum() / normalizer


def parse_args():
    parser = argparse.ArgumentParser()
    parser.add_argument('--datapath', type=str, default='./data/DUTS')
    parser.add_argument('--mask_dir', type=str, default='fusion_larger', choices=['fusion_larger'])
    parser.add_argument('--intersection_dir', type=str, default='fusion_intersection')
    ...
    parser.add_argument('--impure_min_weight', type=float, default=0.9)
    parser.add_argument('--impure_max_weight', type=float, default=1.0)
    parser.add_argument('--num_segments', type=int, default=300)
    parser.add_argument('--compactness', type=float, default=10.0)
    parser.add_argument('--distance_temperature', type=float, default=80.0)
    parser.add_argument('--color_temperature', type=float, default=20.0)
    return parser.parse_args()


def train(Dataset, Network, args):
    exp_name = f"scwssod_{args.mask_dir}_adaptive_weighted"
    ...
    cfg = Dataset.Config(
        ...
        intersection_dir=args.intersection_dir,
        trainsize=args.trainsize,
        alpha=args.alpha,
        impure_min_weight=args.impure_min_weight,
        impure_max_weight=args.impure_max_weight,
    ...
    for epoch in range(cfg.epoch):
        prefetcher = WeightedDataPrefetcher(loader)
        batch_idx = -1
        image, mask, weight_map = prefetcher.next()
        ...
        while image is not None:
            ...
            pixel_weight = weight_map.squeeze(1)
            ...
            loss2_lsc = loss_lsc(
                out2_, loss_lsc_kernels_desc_defaults, loss_lsc_radius,
                sample, image_.shape[2], image_.shape[3]
            )['loss']
            loss2 = (
                loss_ssc
                + weighted_cross_entropy(out2, fg_label, pixel_weight)
                + weighted_cross_entropy(out2, bg_label, pixel_weight)
                + l * loss2_lsc
            )

            out3_ = F.interpolate(out3[:, 1:2], scale_factor=0.25, mode='bilinear', align_corners=True)
            loss3_lsc = loss_lsc(
                out3_, loss_lsc_kernels_desc_defaults, loss_lsc_radius,
                sample, image_.shape[2], image_.shape[3]
            )['loss']
            loss3 = (
                weighted_cross_entropy(out3, fg_label, pixel_weight)
                + weighted_cross_entropy(out3, bg_label, pixel_weight)
                + l * loss3_lsc
            )

            out4_ = F.interpolate(out4[:, 1:2], scale_factor=0.25, mode='bilinear', align_corners=True)
            loss4_lsc = loss_lsc(
                out4_, loss_lsc_kernels_desc_defaults, loss_lsc_radius,
                sample, image_.shape[2], image_.shape[3]
            )['loss']
            loss4 = (
                weighted_cross_entropy(out4, fg_label, pixel_weight)
                + weighted_cross_entropy(out4, bg_label, pixel_weight)
                + l * loss4_lsc
            )

            out5_ = F.interpolate(out5[:, 1:2], scale_factor=0.25, mode='bilinear', align_corners=True)
            loss5_lsc = loss_lsc(
                out5_, loss_lsc_kernels_desc_defaults, loss_lsc_radius,
                sample, image_.shape[2], image_.shape[3]
            )['loss']
            loss5 = (
                weighted_cross_entropy(out5, fg_label, pixel_weight)
                + weighted_cross_entropy(out5, bg_label, pixel_weight)
                + l * loss5_lsc
            )
...
if __name__ == '__main__':
    args = parse_args()
    train(fusion_adaptive_weighted_dataset, SCWSSOD, args)
